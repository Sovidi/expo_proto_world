...
const ContentBox = ({ scrollScanner, item }) => {
  const { targetRef, isViewed, handleScroll } = useIntersectionObserver();
  ...
  return (
    <View ref={targetRef} className={`${isViewed ? "!bg-[blue]" : "!bg-[red]"} flex-row justify-start items-center w-full rounded-[8px] mb-10 bg-[white]`}>
      <MotiView from={{ opacity: 0 }} animate={isViewed ? { opacity: 1 } : { opacity: 0 }} transition={{ type: "timing", duration: 500 }}>
